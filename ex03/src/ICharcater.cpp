...
/*   Updated: 2025/02/19 23:43:12 by tcohen           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ICharacter::ICharacter(const ICharacter &copy)
{
	...
	this->_ThrashSize = copy._ThrashSize;
	this->_inventorySize = copy._inventorySize;
	for(int i = 0; i <= copy._inventorySize; i++)
		this->_inventory[i] = copy._inventory[i];
	if (copy._MateriaThrash == NULL)
		this->_MateriaThrash = NULL;
	if (copy._MateriaThrash != NULL)
	{
		if ((this->_MateriaThrash = new(std::nothrow) AMateria*[copy._ThrashSize]) == NULL)
		{
			this->_error = 1;
			return;
		}
		for (int i = 0; i < copy._ThrashSize; i++)
			this->_MateriaThrash[i] = copy._MateriaThrash[i];
	}
